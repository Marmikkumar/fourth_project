...
export default function Navbar(props) {
  const updlangHindi = (event) => {
    const newlang = event.target.value;
    if (newlang === "English") props.onUpdatelang(true);
    else props.onUpdatelang(false);
  };
  ...
  return (
    <div className={props.val ? "navbar" : "non"}>
      <div className="menu">
        <img
        ...
        <div className="menubar">
          <div className="box">
            ...
            <select onClick={updlangHindi} name="lang" id="">
              <option value="English">English</option>
              <option value="Hindi">हिंदी</option>
            </select>
          </div>
          <button className="btn" onClick={handlebtn} type="submit">
            <Link className="linkbtn" to="/signin">
              {props.val2 ? "Sign In" : "साइन इन"}
            </Link>
          </button>
        </div>
      </div>
